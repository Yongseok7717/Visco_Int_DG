...
psix1 = Expression(("-varphi1*x[0]*x[1]*(tau1*exp(-tn + 1)/(tau1-1)-tau1*exp(-tn/tau1+1)/(tau1 - 1))/tau1","-varphi1*(tau1*exp(-tn/tau1)/(tau1*tau1 + 1) - (tau1*tau1*sin(tn) + tau1*cos(tn))/(tau1*tau1 + 1))*sin(x[0]*x[1])/tau1"),tau1=tau1, varphi1=varphi1,  tn=0, degree=5)
#[ p1=1-exp(-tn/tau1) q1=(tn-tau1+tau1*exp(-tn/tau1)) ]

psix2 = Expression(("-varphi2*x[0]*x[1]*(tau2*exp(-tn + 1)/(tau2 - 1) - tau2*exp(-tn/tau2 + 1)/(tau2 - 1))/tau2","-varphi2*(tau2*exp(-tn/tau2)/(tau2*tau2 + 1) - (tau2*tau2*sin(tn) + tau2*cos(tn))/(tau2*tau2 + 1))*sin(x[0]*x[1])/tau2"),tau2=tau2, varphi2=varphi2,  tn=0, degree=5)
...
# assemble the system matrix once and for all
        ...
        CurlA=0.5*(1.0-(varphi1/(tau1/dt+0.5)/2+varphi2/(tau2/dt+0.5)/2))*A
...
# assemble only once, before the time stepping
        ...
        for n in range(0,Nt):
         #   progbar += 1;
            ...
            solver = LUSolver('mumps')
            bc.apply(B,b) # not essentially required
            
            solve(B, uh.vector(), b)   
            wh.assign(2.0/dt*(uh-u0)-w0)
            psih1.assign(2.0*dt/(2*tau1+dt)*((2*tau1-dt)/2/dt*psi01+varphi1/2.0*(uh+u0)))
            psih2.assign(2.0*dt/(2*tau2+dt)*((2*tau2-dt)/2/dt*psi02+varphi2/2.0*(uh+u0))) 
...
if m>2:    
    ...
    if k==1:        
        np.savetxt("L2_error_linear_velo_S1.txt",L2_error,fmt="%2.3e")
        np.savetxt("L2_error_linear_disp_S1.txt",Lu_error,fmt="%2.3e")
        np.savetxt("H1_error_linear_velo_S1.txt",H1_error,fmt="%2.3e")
        np.savetxt("H1_error_linear_disp_S1.txt",Hu_error,fmt="%2.3e")
        
    elif k==2:
        np.savetxt("L2_error_quad_velo_S1.txt",L2_error,fmt="%2.3e")
        np.savetxt("L2_error_quad_disp_S1.txt",Lu_error,fmt="%2.3e")
        np.savetxt("H1_error_quad_velo_S1.txt",H1_error,fmt="%2.3e")
        np.savetxt("H1_error_quad_disp_S1.txt",Hu_error,fmt="%2.3e")
if jMax-jMin==1:
